...
export const GET: RequestHandler = async ({ url }) => {
  ...
  if (type === 'diseases') {
    ...
    countQ = `SELECT COUNT(*) FROM kb_diseases WHERE 1=1${q?' AND (name_zh LIKE ? OR canonical_name LIKE ? OR slug LIKE ?)':''}${species?' AND species LIKE ?':''}${category?' AND category=?':''}`;
  } else if (type === 'drugs') {
    ...
    countQ = `SELECT COUNT(*) FROM kb_drugs2 WHERE 1=1${q?' AND (name_zh LIKE ? OR generic_name LIKE ? OR brand_names LIKE ?)':''}`;
  } else if (type === 'breeds') {
    ...
    countQ = `SELECT COUNT(*) FROM kb_breeds2 WHERE 1=1${q?' AND (name_zh LIKE ? OR canonical_name LIKE ?)':''}${species?' AND species=?':''}`;
  } else if (type === 'symptoms') {
    base = 'SELECT slug,canonical_name,name_zh,disease_count FROM kb_symptoms WHERE 1=1';
    if (q) { base += ' AND (name_zh LIKE ? OR canonical_name LIKE ?)'; params.push(`%${q}%`, `%${q}%`); }
    countQ = `SELECT COUNT(*) FROM kb_symptoms WHERE 1=1${q?' AND (name_zh LIKE ? OR canonical_name LIKE ?)':''}`;
  } else {
  ...
  base += ' ORDER BY name_zh ASC LIMIT ? OFFSET ?';
