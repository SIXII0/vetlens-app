...
<div class="max-w-4xl mx-auto space-y-6">
  ...
  <div class="flex gap-2 flex-wrap">
    {#each Object.entries(REPORT_LABELS) as [type, { icon, label }]}
      <button
        ...
        class:bg-brand-50={typeFilter === type}
        class:border-brand-300={typeFilter === type}
        class:text-brand-700={typeFilter === type}
        class:bg-white={typeFilter !== type}
        class:border-warm-200={typeFilter !== type}
        class:text-warm-600={typeFilter !== type}
        class:hover:bg-warm-50={typeFilter !== type}
  ...
  {:else if reports.length === 0}
    <div class="card text-center py-12">
      ...
      <h3 class="font-semibold text-warm-700 mb-2">暂无分析报告</h3>
      <p class="text-sm text-warm-500 mb-4">上传账单并选择报告格式即可生成结构化报告</p>
        ...
        <div class="card-hover flex items-center gap-4">
          ...
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2">
              <a href="/reports/{report.id}" class="font-medium truncate hover:text-brand-600 text-warm-900 no-underline">
                {report.title}
              </a>
            ...
            <div class="text-xs text-warm-500">
              ...
              {/if}
              {#if report.recordId}
                · <a href="/analysis/{report.recordId}" class="text-brand-500 hover:underline">查看就诊记录</a>
              {/if}
            </div>
            <div class="text-xs text-warm-400 mt-0.5">
