...
{#if loading}
  <div class="max-w-4xl mx-auto py-12 text-center text-gray-400">加载中...</div>
{:else if !pet}
  <div class="max-w-4xl mx-auto py-12 text-center text-gray-400">宠物不存在</div>
{:else}
<div class="max-w-4xl mx-auto space-y-6">
  <!-- 顶部栏 -->
  <div class="flex items-center gap-3">
    <button class="text-gray-400 hover:text-gray-600 text-lg" onclick={() => goto('/pets')}>←</button>
  ...
  <div class="flex gap-1 bg-gray-100 rounded-lg p-1">
    {#each [{k:'overview',l:'💰 花费总览'},{k:'records',l:'📋 就诊记录'},{k:'reports',l:'📄 医疗报告'}] as tab}
      <button
        class="flex-1 py-2 rounded-md text-sm font-medium transition-colors {activeTab === tab.k ? 'bg-white text-gray-900 shadow-sm' : 'text-gray-500 hover:text-gray-700'}"
  ...
  {#if activeTab === 'overview' && spending}
    ...
    <div class="flex items-center gap-2">
      <button class="text-gray-400 hover:text-gray-600" onclick={() => selectedYear--} disabled={selectedYear <= 2020}>‹</button>
      <span class="font-semibold text-gray-900">{selectedYear}年</span>
      <button class="text-gray-400 hover:text-gray-600" onclick={() => selectedYear++} disabled={selectedYear >= new Date().getFullYear()}>›</button>
    </div>

    <div class="grid grid-cols-3 gap-4">
      <div class="card text-center">
        <div class="text-2xl font-bold text-gray-900">{formatCurrency(spending.annualTotal)}</div>
        <div class="text-xs text-gray-500 mt-1">年度总花费</div>
      </div>
      <div class="card text-center">
        <div class="text-2xl font-bold text-gray-900">{spending.visitCount} 次</div>
        <div class="text-xs text-gray-500 mt-1">就诊次数</div>
      </div>
      <div class="card text-center">
        <div class="text-2xl font-bold text-gray-900">{formatCurrency(spending.avgPerVisit)}</div>
        <div class="text-xs text-gray-500 mt-1">单次均价</div>
      </div>
    </div>
    ...
    {#if spending.annualTotal > 0}
      <div class="card">
        <h3 class="font-semibold text-gray-700 mb-4">📊 月度花费趋势</h3>
        ...
        <div class="flex items-end gap-0.5 h-32">
          {#each spending.monthlyTrend as m}
            <div class="flex-1 flex flex-col items-center justify-end h-full group relative">
              <div
                class="w-full rounded-t-sm bg-primary-400 hover:bg-primary-500 transition-all cursor-pointer"
                style="height: {barHeight(m.total, maxMonthlyTotal)}"
                title="{m.month}月: {formatCurrency(m.total)}"
              ></div>
              {#if m.total > 0}
                <div class="hidden group-hover:block absolute -top-7 bg-gray-800 text-white text-xs px-2 py-0.5 rounded whitespace-nowrap z-10">
      ...
      <!-- 费用构成环形图 -->
      <div class="card">
        <h3 class="font-semibold text-gray-700 mb-4">🎨 费用构成</h3>
        <div class="space-y-2">
          {#each spending.categoryBreakdown.slice(0, 6) as cat}
            <div class="flex items-center gap-3">
              <div class="w-3 h-3 rounded-full flex-shrink-0" style="background: {getCategoryColor(cat.category)}"></div>
              <span class="text-sm w-12 flex-shrink-0">{cat.category}</span>
              ...
              <span class="text-sm text-gray-500 w-12 text-right">{formatCurrency(cat.total)}</span>
    ...
    {#if spending.insurance?.hasPolicy}
      ...
      <div class="card border-emerald-200 bg-emerald-50/50">
        <h3 class="font-semibold text-gray-700 mb-3">🛡️ 保险保障</h3>
        <div class="text-sm text-gray-600 mb-2">{ins.company} — {ins.productName}</div>
        <div class="grid grid-cols-3 gap-3 text-center text-sm">
          <div class="bg-white rounded-lg p-2">
            <div class="font-semibold">{formatCurrency(ins.totalSpent)}</div>
            <div class="text-xs text-gray-400">总花费</div>
          </div>
          <div class="bg-white rounded-lg p-2">
            <div class="font-semibold text-emerald-600">{formatCurrency(ins.estimatedPayout)}</div>
            <div class="text-xs text-gray-400">预计赔付</div>
          </div>
          <div class="bg-white rounded-lg p-2">
            <div class="font-semibold text-amber-600">{formatCurrency(ins.netOutOfPocket)}</div>
            <div class="text-xs text-gray-400">实际自付</div>
          </div>
        </div>
    ...
    {:else if spending.annualTotal > 0}
      <div class="card text-center text-sm text-gray-500 py-4">
        💡 <button class="text-primary-500 underline" onclick={() => goto('/insurance')}>添加保单</button>后可查看预计赔付和实际自付
      </div>
    {/if}
    ...
    {#if policies.length > 0}
      <div class="card">
        <h3 class="font-semibold text-gray-700 mb-3">📜 所有保单</h3>
        <div class="space-y-2">
          {#each policies as pol}
            <div class="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
              <div>
                <div class="text-sm font-medium text-gray-800">{pol.company} — {pol.product_name}</div>
                <div class="text-xs text-gray-400">
                  {#if pol.effective_date}生效: {pol.effective_date}{/if}
                  {#if pol.expiry_date} · 到期: {pol.expiry_date}{/if}
                </div>
              </div>
              <span class="text-xs px-2 py-0.5 rounded-full {pol.status === 'active' ? 'bg-emerald-100 text-emerald-700' : 'bg-gray-100 text-gray-500'}">
  ...
  {:else if activeTab === 'records'}
    {#if records.length === 0}
      <div class="text-center py-12 text-gray-400">暂无就诊记录</div>
    {:else}
      <div class="space-y-3">
        {#each records as rec}
          <div class="card cursor-pointer hover:shadow-md transition-all"
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="font-medium text-gray-900">{rec.hospital_name || '未知医院'}</div>
                <div class="text-sm text-gray-500">{rec.visit_date}</div>
                {#if rec.diagnosis}
                  <div class="text-xs text-gray-400 mt-0.5">诊断: {rec.diagnosis}</div>
                {/if}
              </div>
              <div class="text-right">
                <div class="font-semibold text-gray-900">{formatCurrency(rec.total_amount)}</div>
                <div class="text-xs text-gray-400">{rec.status === 'analyzed' ? '已分析' : rec.status}</div>
              </div>
            </div>
          </div>
        {/each}
  ...
  {:else if activeTab === 'reports'}
    {#if reports.length === 0}
      <div class="text-center py-12 text-gray-400">
        暂无医疗报告<br>
        <span class="text-xs text-gray-300">分析就诊记录后可生成报告</span>
      </div>
    {:else}
      <div class="space-y-3">
        {#each reports as rpt}
          <div class="card cursor-pointer hover:shadow-md transition-all"
            ...
            <div class="flex items-center justify-between">
              <div>
                <div class="font-medium text-gray-900">{rpt.title || '医疗报告'}</div>
                <div class="text-sm text-gray-500">{rpt.report_type} · {new Date(rpt.created_at).toLocaleDateString('zh-CN')}</div>
              </div>
              <div class="flex items-center gap-2">
